...
namespace Rpo.ApiServices.Api.Controllers.CronJobs
{
    ...
    {
        [HttpGet]
        [Route("api/CronJobs/GetECBCronJob/{binnumber}")]
                    ...
                    if (resultsBIS != null && resultsBIS.Count() > 0)
                    {
                        foreach (var item in resultsBIS)
                        {
                            ...
                            if (jobViolation == null)
                            {
                                ...
                                {
                                    throw new RpoBusinessException("No longer scrape dummy BIN’s " + records.bin);
                                }
                            }
                        }
                    }
        ...
        [HttpGet]
        [Route("api/CronJobs/GetDOBCronJob/{binnumber}")]
        public void SendDOBNotification(string binnumber)
        {
            ...
            using (var ctx = new Model.RpoContext())
            {
                    ...
                    foreach (var item in lstDOBViolationsResponse)
                    {
                        ...
                        if (item.Value == true)
                        {
                            if (firstaddress != null)
                            {
                                ...
                                NewDOBViolations += "<p>Violation <b> " + item.Key + "</b> has been issued at <b>" + (!string.IsNullOrWhiteSpace(firstaddress.HouseNumber) ? firstaddress.HouseNumber : "") + " "
                            ...
                            {
                                project += "<b><a href=\"https://rpo.credencys.net/job/" + jobid + "/violation?highlighted=" + item.Key + "&isDob=true" + "\">" + jobid + "</a></b>,";
                                //  project += "<b><a href=\"https://rpo.credencys.net/job/" + jobid + "/violation\">" + jobid + "</a></b>,";
                            }
                ...
                if (to != null && to.Count > 0)
                {
                    ...
                    {
                        Mail.Send(new KeyValuePair<string, string>(Properties.Settings.Default.SmtpUserName, "RPO APP"), to, cc, commonsub + "New DOB Violations Issued - Action may be required", NewemailBody, true);
                        SendNewMailPersonWise("DOB", lstDOBViolationsResponse);
                    }
                    if (DOBViolations != null && DOBViolations != "")
                    {
                        ...
                        SendUpdateMailPersonWise("DOB", lstDOBViolationsResponse);
                    }
                }
            }
        }
        [HttpGet]
        [Route("api/CronJobs/GetViolationCronJob/{violation}")]
        public void ViolationCronJob(string violation)
        {
            RpoContext rpoContext = new RpoContext();
            string body = string.Empty;
            string emailtemplatepath = AppDomain.CurrentDomain.BaseDirectory + "EmailTemplate/violationMailerBinChange.html";
            using (StreamReader reader = new StreamReader(emailtemplatepath))
            {
                body = reader.ReadToEnd();
            }
            List<string> violationLIst = new List<string>();
            violationLIst.Add("39517468K");
            violationLIst.Add("39056894Y");
            violationLIst.Add("39522910K");
            string emailBody = body;
            string violationbody = null;
            var violatioList = (from v in rpoContext.JobViolations
                                ...
                                // where v.SummonsNumber == violation.TrimEnd() && job.Status == JobStatus.Active && v.IsFullyResolved == false
                                where violationLIst.Contains(v.SummonsNumber) && job.Status == JobStatus.Active && v.IsFullyResolved == false
                                select v.SummonsNumber).Distinct().AsQueryable();
            var allViolations = rpoContext.JobViolations.Where(x => violatioList.Contains(x.SummonsNumber)).ToList();

            string commanMessage = string.Empty;
            foreach (var SummonsNumber in allViolations)
            {
                ...
                var rowsBIS = datasetBIS.GetRows(limit: 1000);
                string qryBIS = string.Empty;
                qryBIS = qryBIS + "number='" + SummonsNumber.SummonsNumber + "'" + " and " + "violation_category  like '" + "%ACTIVE%" + "'";

                var binNo = new SoqlQuery().Select("bin").Where(qryBIS);
                var resultsBIS = datasetBIS.Query(binNo);
                if (resultsBIS != null && resultsBIS.Count() > 0)
                {
                    foreach (var item in resultsBIS)
                    {
                        ...
                        if (records.bin != null && !string.IsNullOrEmpty(records.bin) && (string.IsNullOrEmpty(SummonsNumber.BinNumber) || SummonsNumber.BinNumber.ToLower().Trim() != records.bin.ToLower().Trim()))
                        {
                            //SummonsNumber.BinNumber = Convert.ToString(records.bin);
                            //SummonsNumber.LastModifiedBy = 2;
                            //SummonsNumber.LastModifiedDate = DateTime.UtcNow;
                            //SummonsNumber.IsUpdateMailsent = false;
                            //commanMessage = commanMessage + " " + "Bin Number was updated to <'" + records.bin + "'>.";
                            //violationbody += "Violation number: " + SummonsNumber.SummonsNumber + "<br>";
                            //var idrfps = rpoContext.RfpAddresses.Where(x => x.BinNumber == SummonsNumber.BinNumber).Select(x => x.Id).ToList();
                            //var jobs = rpoContext.Jobs.Where(x => idrfps.Contains(x.IdRfpAddress)).ToList();
                            //string joblist = null;
                            //foreach (var j in jobs)
                            //{
                            //    string job = "Project : " + j.JobNumber +" " +"Address : " + j.HouseNumber + " " + j.StreetNumber + " " + j.Borough + " " + "Binumber : " + SummonsNumber.BinNumber + "<br>";
                            //    joblist += job;
                            //}
                            //violationbody += joblist;
                            //violationbody += "New Bin detected : " + records.bin.Trim() + "<br><br>";
                            violationbody += "Violation number: <b>" + SummonsNumber.SummonsNumber + "</b><br>";
                            var idrfps = rpoContext.RfpAddresses.Where(x => x.BinNumber == SummonsNumber.BinNumber).Select(x => x.Id).ToList();
                            var jobs = rpoContext.Jobs.Where(x => idrfps.Contains(x.IdRfpAddress)).ToList();
                            //&& x.Status== JobStatus.Active).ToList();
                            string joblist = null;
                            foreach (var j in jobs)
                            {
                                string job = "Project : <b>" + j.JobNumber + "</b> " + "Address : " + j.HouseNumber + " " + j.StreetNumber + " " + j.Borough.Description + " " + "Binumber : " + SummonsNumber.BinNumber + "<br>";
                                joblist += job;
                            }
                            violationbody += joblist;
                            violationbody += "New Bin detected :<b> " + records.bin.Trim() + "</b><br><br>";

                        }
                        // rpoContext.Entry(SummonsNumber).State = EntityState.Modified;
                        //if (!string.IsNullOrEmpty(commanMessage))
                        //{
                ...
                var clientECB = new SodaClient("https://data.cityofnewyork.us", "7tPQriTSRiloiKdWIJoODgReN");
                var datasetECB = clientECB.GetResource<object>("6bgk-3dad");
                ServicePointManager.Expect100Continue = true;
                ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls |
                  SecurityProtocolType.Tls11 |
                  SecurityProtocolType.Tls12;
                var rowsECB = datasetECB.GetRows(limit: 1000);
                string qryECB = string.Empty;
                qryECB = qryECB + "ecb_violation_number='" + SummonsNumber.SummonsNumber + "'" + " and " + "ecb_violation_status='" + "ACTIVE" + "'";
                var soqlECB = new SoqlQuery().Select("bin").Where(qryECB);

                var resultECB = datasetECB.Query(soqlECB);

                if (resultECB != null && resultECB.Count() > 0)
                {

                    foreach (var item in resultECB)
                    {
                        string jsonstring = JsonConvert.SerializeObject(item);
                        var records = JsonConvert.DeserializeObject<ViolationDOBJsonRes>(jsonstring);
            ...
            //    }
            //}
            //to.Add(new KeyValuePair<string, string>("mpressel@rpoinc.com", "Michael" + " " + "Pressel"));
            to.Add(new KeyValuePair<string, string>("annapurna.bingi@credencys.com", "Kevin" + " " + "Danielson"));
            to.Add(new KeyValuePair<string, string>("meethalal.teli@credencys.com", "Robert" + " " + "Anic"));
            to.Add(new KeyValuePair<string, string>("mital.bhatt@credencys.com", "Michael" + " " + "Pressel"));

            List<KeyValuePair<string, string>> cc = new List<KeyValuePair<string, string>>();
            if (to != null && to.Count > 0)
            {
                if (violationbody != null && violationbody != "")
                {
                    emailBody = emailBody.Replace("##ViolationNumber##", violationbody);
                    Mail.Send(new KeyValuePair<string, string>("qa1@credencys.net", "Credencys"), to, cc, commonsub + "Mismatch in Bin number detected in violation ", emailBody, true);

                }
            }
        }
             
        public void UpdateECBCronJob(List<string> lstECBViolationse, Dictionary<string, bool> lstECBViolationsResponse)
        {
            StringBuilder sb = new StringBuilder();
            using (var ctx = new Model.RpoContext())
            {
                    ...
                    foreach (var item in lstECBViolationsResponse)
                    {
                        ...
                        if (item.Value == true)
                        {
                            if (firstaddress != null)
                            {
                                ...
                                {
                                    NewECBViolations += "<p>Violation <b> " + item.Key + "</b> has been issued at <b>" + (!string.IsNullOrWhiteSpace(firstaddress.HouseNumber) ? firstaddress.HouseNumber : "") + " "
                                                    + (!string.IsNullOrWhiteSpace(firstaddress.Street) ? firstaddress.Street : "") + " " + (!string.IsNullOrWhiteSpace(boroughes) ? boroughes : "") + " "
                                                    + (!string.IsNullOrWhiteSpace(firstaddress.ZipCode) ? firstaddress.ZipCode : "") + "</b> , Issuing Agency : " + (!string.IsNullOrWhiteSpace(issuingagency) ? issuingagency : "")
                                                    + ", Violation Description : " + (!string.IsNullOrWhiteSpace(descritption) ? descritption : "");
                                }
                ...
                if (to != null && to.Count > 0)
                {
                    ...
                    {
                        Mail.Send(new KeyValuePair<string, string>(Properties.Settings.Default.SmtpUserName, "RPO APP"), to, cc, commonsub + "New ECB/OATH Violations Issued - Action may be required", NewemailBody, true);
                        SendNewMailPersonWise("ECB", lstECBViolationsResponse);
                    }
                    if (DOBViolations != null && DOBViolations != "")
                    {
                        ...
                        SendUpdateMailPersonWise("ECB", lstDOBViolationsResponse);
                    }
        ...
        public static void SendViolationUpdatedSNotification(List<int> jobNumbers, string violationNo, bool isNotExits, bool ViolationType, string commonmessage)
        {
            ...
            foreach (var item in employeelist)
            {
                ...
                else
                {
                    ...
                    //.Replace("##RedirectionLinkForViolation##", Properties.Settings.Default.FrontEndUrl + "/job/" + lnkJobNumbers + "/violation");
                    Common.SendInAppNotifications(employee.Id, newJobScopeAddedSetting, "/job/" + jobNumbers.FirstOrDefault() + "/scope");
                }
        ...
        public static void SendNewMailPersonWise(string Type, Dictionary<string, bool> lstECBViolationsResponse)
        {
            string body = string.Empty;
            string emailtemplatepath = AppDomain.CurrentDomain.BaseDirectory + "EmailTemplate/violationMailer.html";
            using (StreamReader reader = new StreamReader(emailtemplatepath))
            ...
            string NewemailBody = body;
            string NewCustomeremailBody = body;
            var rpoContext = new Model.RpoContext();
            ...
            foreach (var TeamMember in jobAssignList.Distinct())
            {
                ...
                if (Type == "ECB")
                {
                    //finalviolations = rpoContext.JobViolations.Where(x => binnumbers.Contains(x.BinNumber) && x.IsUpdateMailsent == false && x.Type_ECB_DOB == "ECB" && x.PartyResponsible == 1).ToList();
                ...
                foreach (var item in finalviolations)
                {
                    ...
                    if (firstaddress != null)
                    {
                        ...
                        if (!string.IsNullOrWhiteSpace(issuingagency) && !string.IsNullOrWhiteSpace(descritption))
                        {
                            NewECBViolations += "<p>Violation <b> " + item.SummonsNumber + "</b> has been issued at <b>" + (!string.IsNullOrWhiteSpace(firstaddress.HouseNumber) ? firstaddress.HouseNumber : "") + " "
                                            + (!string.IsNullOrWhiteSpace(firstaddress.Street) ? firstaddress.Street : "") + " " + (!string.IsNullOrWhiteSpace(boroughes) ? boroughes : "") + " "
                                            + (!string.IsNullOrWhiteSpace(firstaddress.ZipCode) ? firstaddress.ZipCode : "") + "</b> , Issuing Agency : " + (!string.IsNullOrWhiteSpace(issuingagency) ? issuingagency : "")
                                            + ", Violation Description : " + (!string.IsNullOrWhiteSpace(descritption) ? descritption : "");
                        }
        ...
        public static void SendUpdateMailPersonWise(string Type, Dictionary<string, bool> lstDOBViolationsResponse)
        {
            var rpoContext = new Model.RpoContext();
        ...
        {
            string errorLogFilename = "ViolationLog_" + DateTime.Now.ToString("dd-MM-yyyy") + ".txt";
