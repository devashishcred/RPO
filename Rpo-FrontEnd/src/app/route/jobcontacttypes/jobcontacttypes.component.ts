...
declare const $: any
/**
*  This component contains all function that are used in JobContactTypesComponent
* @class JobContactTypesComponent
*/
@Component({
  templateUrl: './jobcontacttypes.component.html',
  styleUrls: ['./jobcontacttypes.component.scss']
})
export class JobContactTypesComponent implements OnInit, OnDestroy {

  /**
  *  jobcontacttypesform add/edit form
  * @property jobcontacttypesform
  */
  @ViewChild('jobcontacttypesform',{static: true})
  private JobContactTypesform: TemplateRef<any>
  ...
  private JobContactTypesId: number
  private search: string
  private showJobContactTypeAddBtn: string = 'hide'
  private showJobContactTypeDeleteBtn: string = 'hide'
  constructor(
    ...
    this.showJobContactTypeAddBtn = this.userRight.checkAllowButton(this.constantValues.ADDMASTERDATA)
    this.showJobContactTypeDeleteBtn = this.userRight.checkAllowButton(this.constantValues.DELETEMASTERDATA)
    ...
    this.specialColumn = new $.fn.dataTable.SpecialColumn([{
      id: 'EDIT',
      title: 'Edit',
      customClass: this.showJobContactTypeAddBtn
    }, {
      id: 'DELETE',
      title: 'Delete',
      customClass: this.showJobContactTypeDeleteBtn
    }], false)
  ...
  */
  ngOnInit() {
    document.title = 'Project Contact Types'
    const vm = this
    this.filter = {} as any
    vm.table = $('#dt-JobContact-types').DataTable({
      ...
      "aaSorting": [],
      ajax: this.JobContactTypesServices.getRecords({
        onData: (data: any) => {
      ...
      rowCallback: ((row: any, data: any, index: any) => {
        ...
        if (this.showJobContactTypeAddBtn == 'hide') {
          $(row).find('.edit-icon').addClass("disabled");
          $(row).find('td').removeClass('clickable');
        }
        if (this.showJobContactTypeDeleteBtn == 'hide') {
          $(row).find('.delete-icon').addClass("disabled");
        }
      }),
      drawCallback: (setting: any) => {
        if (vm.showJobContactTypeAddBtn == "hide" && vm.showJobContactTypeDeleteBtn == 'hide') {
          $('.select-column').hide()
        } else {
          $('.select-column').show()
        }
      ...
      initComplete: () => {
        this.specialColumn
          ...
          .onActionClick((row: any, actionId: any) => {
            ...
            if (actionId == "EDIT") {
              vm.isNew = false
              vm.JobContactTypesId = data.id
              vm.openModalForm(vm.JobContactTypesform, data.id, false)
            }
            if (actionId == "DELETE") {
  ...
  */
  delete(id: number, row: any) {
    return new Promise((resolve, reject) => {
      this.JobContactTypesServices.delete(id).subscribe(r => {
        row.delete()
        resolve(r)
      }, e => {
        reject()
