...
declare const $: any
/**
*  This component contains all function that are used in CompanyTypesComponent
...
export class CompanyTypesComponent implements OnInit, OnDestroy {

  /**
  *  companytypesform add/edit form
  * @property companytypesform
  */
  @ViewChild('companytypesform',{static: true})
  private CompanyTypesform: TemplateRef<any>
  ...
  private CompanyTypesId: number
  private search: string
  private showCompanyTypeAddBtn: string = 'hide'
  private showCompanyTypeDeleteBtn: string = 'hide'
  ...
  constructor(
    ...
    this.showCompanyTypeAddBtn = this.userRight.checkAllowButton(this.constantValues.ADDMASTERDATA)
    this.showCompanyTypeDeleteBtn = this.userRight.checkAllowButton(this.constantValues.DELETEMASTERDATA)


    this.specialColumn = new $.fn.dataTable.SpecialColumn([{
      id: 'EDIT',
      title: 'Edit',
      customClass: this.showCompanyTypeAddBtn
    }, {
      id: 'DELETE',
      title: 'Delete',
      customClass: this.showCompanyTypeDeleteBtn
    }], false)
  ...
  */
  ngOnInit() {
    ...
    vm.table = $('#dt-company-types').DataTable({
      ...
      "aaSorting": [],
      ajax: this.CompanyTypesServices.getRecords({
        onData: (data: any) => {
      ...
      rowCallback: ((row: any, data: any, index: any) => {
        ...
        if (this.showCompanyTypeAddBtn == 'hide') {
          $(row).find('.edit-icon').addClass("disabled");
          $(row).find('td').removeClass('clickable');
        }
        if (this.showCompanyTypeDeleteBtn == 'hide') {
          $(row).find('.delete-icon').addClass("disabled");
        }
      }),
      drawCallback: (setting: any) => {
        if (vm.showCompanyTypeAddBtn == "hide" && vm.showCompanyTypeDeleteBtn == 'hide') {
          $('.select-column').hide()
        } else {
          $('.select-column').show()
        }
      ...
      initComplete: () => {
        this.specialColumn
          ...
          .onActionClick((row: any, actionId: any) => {
            ...
            if (actionId == "EDIT") {
              vm.isNew = false
              vm.CompanyTypesId = data.id
              vm.openModalForm(vm.CompanyTypesform, data.id, false)
            }
            if (actionId == "DELETE") {
  ...
  */
  delete(id: number, row: any) {
    return new Promise((resolve, reject) => {
      this.CompanyTypesServices.delete(id).subscribe(r => {
        row.delete()
        resolve(r)
      }, e => {
        reject()
