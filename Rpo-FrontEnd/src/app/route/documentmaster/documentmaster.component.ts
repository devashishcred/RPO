...
export class DocumentMasterComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('formdocumentmaster', { static: true })
  formdocumentmaster: TemplateRef<any>
  ...
  private rec: any
  private filter: any
  private applicationTypeId: number
  private loading: boolean = false
  private showApplicationTypeAddBtn: string = 'hide'
  ...
  */
  ngOnInit() {
    ...
    this.table = $('#dt-applicationType').DataTable({
      ...
      ajax: this.applicationTypeServices.getDocumentMasterList({
        onData: (data: any) => {
        ...
        this.specialColumn
          ...
          .onActionClick((row: any, actionId: any) => {
            ...
            if (actionId == "EDIT") {
              ...
              vm.openModal(vm.formdocumentmaster, data.id)
            }
            if (actionId == "DELETE") {
  ...
  */
  private openModal(template: TemplateRef<any>, id?: number) {
    ...
    if (this.new) {
      ...
      this.applicationTypeServices.getDocumentMasterById(id).subscribe(r => {
        this.rec = r as Group
        this.new = false
        this.modalRef = this.modalService.show(template, { class: 'modal-lg' })
      })
  ...
  */
  delete(id: number, row: any) {
    return new Promise((resolve, reject) => {
      this.applicationTypeServices.deleteDocumentMasterRecord(id).subscribe(r => {
        row.delete()
        resolve(r)
      }, e => {
        reject()
  ...
  */
  private searchApplication(srch: string) {
