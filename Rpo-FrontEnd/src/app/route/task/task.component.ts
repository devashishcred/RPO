 ...
 */
  ...
  */
  ...
  private idTaskType: number
  private isNew: boolean = false
  private isSendFromTask: boolean = true
  ...
  */
  ...
  ngOnInit() {
    ...
    this.employeeServices.getEmpDropdown().subscribe(r => {
      this.employeesArr = _.sortBy(r, function (r: any) { return r.lastName.toLowerCase(); });
    }, e => {
    })
 ...
 */
  ...
  */
  private openModalForm(template: TemplateRef<any>, id?: number, idTaskType?: any) {
    if (!id) {
      // this.idJobApp = 0
    }
  ...
  * @param {number} id it is optional which contains id if record is in edit mode
  */
  private openAddTaskForm(template: TemplateRef<any>, id?: number, idJob?: number, idTaskType?: any) {
    this.isNew = false
    if (!id) {
      this.isNew = true
    }
  ...
  * @param {number} id it is optional which contains id if record is in edit mode
  */
  private openModalTaskFilter(template: TemplateRef<any>, id?: number) {
    if (!id) {
      // this.idJobApp = 0
    }
    this.modalRef = this.modalService.show(template, { class: 'modal-task-filter', backdrop: 'static', 'keyboard': false })
  }
  ...
  */
  private reloadSearch(filter: any) {
    this.filter = filter
