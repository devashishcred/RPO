...
export class AddTaskComponent implements OnInit {
  ...
  private scopeDDOptions: any
  private jobFeeScheduleItems: any
  private masterServiceItems: any
  private showQtyTextbox: boolean = false
  private showHoursTextbox: boolean = false
  private serviceQtyZero: boolean = false
  private isEdit: boolean = false
  private requireServiceItem: boolean = false
  private workPermitDDSettings: any = {}
  private jobDocumentPermitDDSettings: any = {}
  private showDuration: boolean = false
  private showDateTime: boolean = false
  private filteredObj: any
  private serviceItemId: number
  private resOfJobTypes: any
  private showThedropDownValues: boolean = false
  private LabelType: string = ''
  private showPermitDropDown: boolean = false
  private selectedJobType: any
  private documents: any
  private taskDocuments: any
    ...
    if (this.frommodeule == 'CompanyModule') {
      if(editTime){
        this.CompanyService.getActiveContactOfComDD(this.idCompany).subscribe(r => {
          this.contactList = _.sortBy(r, function (i: any) { return i.itemName.toLowerCase(); });
        })
      }else{
  ...
  getApplicationDD(ele: number) {
    ...
    if (ele == 3) {
      ...
      this.applicationService.getApplicationsDD(this.idJob, ele).subscribe(r => {
        if (r.length > 0) {
          this.application = _.sortBy(r, function (i: any) { return i.jobApplicationTypeName.toLowerCase(); });
        } else {
          this.loading = false
        }
      }, e => {
  ...
  getWorkPermits(e: any, type: string) {
    ...
    if (e) {
      ...
      this.applicationService.getAppWorkPermitDD(e.id).subscribe(r => {
        if (r && r.length > 0) {
          this.workpermit = _.sortBy(r, function (i: any) { return i.itemName.toLowerCase(); });
        }
        this.loading = false
      }, e => {
        this.loading = false
  ...
  getTaskType() {
    return new Promise((resolve, reject) => {
      this.taskServices.getTaskType().subscribe(r => {
        ...
        this.taskType = _.sortBy(r, function (r: any) { return r.name.toLowerCase(); });
        resolve(r)
      }, e => {
        this.loading = false
        reject()
  ...
  */
  getTaskSatus() {
    ...
    this.taskServices.getTaskStatus().subscribe(r => {
      this.taskStatus = _.sortBy(r, function (i: any) { return i.name.toLowerCase(); });
    }, e => {
      this.loading = false
    })
  }
  ...
  */
  getEmployees() {
    ...
    this.employeeServices.getEmpDropdown().subscribe(r => {
      this.employees = _.sortBy(r, function (r: any) { return r.employeeName.toLowerCase(); });
    }, e => {
      this.loading = false
    })
  }
