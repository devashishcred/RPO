...
declare const $: any
/**
*  This component contains all function that are used in DotPenaltyComponent
* @class DotPenaltyComponent
*/
@Component({
  templateUrl: './dotPenalty.component.html',
  styleUrls: ['./dotPenalty.component.scss']
})
export class DotPenaltyComponent implements OnInit, OnDestroy {

  /**
  *  DotPenaltyComponent add/edit form
  * @property DotPenaltyComponent
  */
  @ViewChild('dotpenaltyform', { static: true })
  private dotpenaltyform: TemplateRef<any>
  ...
  private showDotPenaltyAddBtn: string = 'hide'
  private showDotPenaltyDeleteBtn: string = 'hide'
  ...
  constructor(
    ...
    this.showDotPenaltyAddBtn = this.userRight.checkAllowButton(this.constantValues.ADDMASTERDATA)
    this.showDotPenaltyDeleteBtn = this.userRight.checkAllowButton(this.constantValues.DELETEMASTERDATA)

    this.specialColumn = new $.fn.dataTable.SpecialColumn([{
      id: 'EDIT_TITLE',
      title: 'Edit',
      customClass: this.showDotPenaltyAddBtn
    }, {
      id: 'DELETE_TITLE',
      title: 'Delete',
      customClass: this.showDotPenaltyDeleteBtn
    }], false)
  ...
  */
  ngOnInit() {
    document.title = 'DOT Penalty Schedule'
    const vm = this
    this.filter = {} as any
    vm.table = $('#dt-dotpenalty').DataTable({
      aaSorting: [[0, 'asc']],
      paging: true,
      ...
      ajax: this.dotPenaltyServices.getRecords({
        onData: (data: any) => {
      ...
      initComplete: () => {
        this.specialColumn
          ...
          .onActionClick((row: any, actionId: any) => {
            ...
            if (actionId == "EDIT_TITLE") {
              vm.isNew = false
              vm.id = data.id
              vm.openModalForm(vm.dotpenaltyform, data.id, false)
            }
            if (actionId == "DELETE_TITLE") {
              this.appComponent.showDeleteConfirmation(this.delete, [data.id, row])
            }
          })
        $('#dt-dotpenalty tbody').on('click', 'span', function (ev: any) {
        ...
        $('#dt-dotpenalty tbody').on('click', 'td.clickable', function (ev: any) {
          const row = vm.table.row($(this).parents('tr'))
          const data = row.data()
  ...
  */
  ngOnDestroy() {
    $('#dt-dotpenalty tbody').off('click')
    $('#dt-dotpenalty').off('draw.dt')
  }
  ...
  */
  private searchDOTPenalty(srch: string) {
  ...
  */
  private openModalForm(template: TemplateRef<any>, id?: number, isNew?: boolean) {
    ...
    this.modalRef = this.modalService.show(template, { class: 'modal-dot-penalty', backdrop: 'static', 'keyboard': false })
  }
  ...
  */
  delete(id: number, row: any) {
    return new Promise((resolve, reject) => {
      this.dotPenaltyServices.delete(id).subscribe(r => {
        row.delete()
        resolve(r)
      }, e => {
        reject()
