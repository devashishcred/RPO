...
export class CompanyComponent implements OnInit, OnDestroy {
  ...
  private showCompanyExportBtn: string = 'hide'
  //Conact show hide
  ...
  private jobOfContact: Job
  private selectUndefinedOptionValue: any;
  private modalRef: BsModalRef
  private new: boolean = true
  private isConsulting: boolean = true
  private rec: Company
  ...
  */
    ...
    if (companyTypeText != '') {
      this.companyServices.getBusinessFromBis(businessName, companyTypeText, licenceNumber).subscribe(data => {
        ...
        if (data && data.length >= 1) {
          this.modalRef = this.modalService.show(template, { class: 'modal-get-info', backdrop: 'static', 'keyboard': false })
          this.loading = false;
        } else {
          this.loading = false;
   ...
   */
  private onSave(comp: Company, evt: any) {
  ...
  */
   ...
   * @param {number} id ID 
   */
  private openModalFormAddTask(template: TemplateRef<any>, id?: number, from?: string) {
    if (from) {
