...
export class FormJobComponent implements OnInit {
  ...
  */
  @ViewChild('formJobAddNewAddress',{static: false})
  ...
  */
  ...
  private projectManager: Employee[] = []
  private projectCoordinator: Employee[] = []
  ...
  private applicationsCtr: number = 0
  private touchedType: boolean = false
  private showDot: boolean = false
  private showDob: boolean = false
  private showViolation: boolean = true
  private showDep: boolean = false
  private dotProjectTeam: any = []
  private dobProjectTeam: any = []
  private violationProjectTeam: any = []
  private depProjectTeam: any = []
  private dobdropdownSettings: any = {};
  private dotdropdownSettings: any = {};
  private depdropdownSettings: any = {};
  private violationdropdownSettings: any = {};
  ...
  * @param {number} id it is optional which contains id if record is in edit mode
  */
  private openModalAddAdress(template: TemplateRef<any>, id?: number) {
  ...
  */
   ...
   */
  private setAddressInDropDownList(addressAndJobInfo:any){
    this.rfpAddressList = []
...
/**
  ...
  * @method setContacts
  */
  private setContacts(comp?:string, editTime?:number) {
    if (comp){
      if(!this.rec.idCompany){
...
/**
  ...
  */
 private setContactsreferrel(comp?:string, editTime?:number) {
  if (comp){
  ...
  if(this.rec.id && this.rec.idReferredByContact && !editTime){
    this.contactServices.getrfpContactDropdown().subscribe(r => {
      if (r && r.length > 0) {
        this.contactsList = _.sortBy(r, function (i) { return i['itemName'].toLowerCase(); });
        if(this.rec.idReferredByCompany)
        {
...
/**
  ...
  */
 private setContactsreferred(comp?:string, editTime?:number) {
  this.contactsList = this.contactsList.filter(x => x.idCompany == this.rec.idReferredByCompany);
}
  ...
  */
  private removeJobType(jobId: number, event: any) {
