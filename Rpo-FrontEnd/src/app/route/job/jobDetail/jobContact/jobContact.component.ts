 ...
 */
   ...
   * @param {number} id ID 
   */
  private openModalAddJobContact(template: TemplateRef<any>, id?: number, jobID?: number) {
    if (jobID) {
      this.idJob = jobID;
    }
    this.isNew = true
  ...
  /**
     ...
     * @param {number} id ID 
     */
  private openModalManageGroup(template: TemplateRef<any>, id?: number) {
    this.isNew = true
    if (id) {
      this.isNew = false
    }
