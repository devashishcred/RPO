...
export class AddJobContactComponent {
  ...
  private contactTypedropdownList: any = [];
  ...
  */
  private getContact(selectCompany: number, editTime?: string, contact?: number) {
    this.loading = true
    ...
    if (selectCompany != null) {
      ...
      this.JobContactService.getAllAddress(selectCompany, null).subscribe(r => {
        ...
        for (let i = 1; i < this.contactAddress.length; i++) {
          this.contactAddressdropdownList.push({ id: this.contactAddress[i].id, itemName: '[' + this.contactAddress[i].addressType.name + '] ' + this.contactAddress[i].address1 + ', ' + (this.contactAddress[i].address2 != null ? this.contactAddress[i].address2 + ', ' : "") + (this.contactAddress[i].city != null ? this.contactAddress[i].city + ', ' : "") + this.contactAddress[i].state + ' ' + (this.contactAddress[i].zipCode != null ? this.contactAddress[i].zipCode : "") + '|' + (this.contactAddress[i].phone != null ? this.contactAddress[i].phone : '-') });
        }
      })
  ...
  */
  private getContactAddress(selectCompany: number, selectContact: number, contact?: string) {
    if (contact) {
      let num = 1;
