...
* @class FormAddDobViolation
*/
@Component({
  selector: '[add-dob-violation]',
  templateUrl: './formAddDobViolation.component.html',
  styleUrls: ['./formAddDobViolation.component.scss'],
})
export class FormAddDobViolation {
  ...
  */
  ...
  getData() {
    this.loading = true;
    this.jobViolationServices.getDobVioationById(this.idViolation).subscribe(res => {
      console.log(res)
      this.violation = res
      if (this.violation.dateIssued) {
        this.violation.dateIssued = this.taskServices.dateFromUTC(this.violation.dateIssued, true);
      }
      if (this.violation.dispositionDate) {
        this.violation.dispositionDate = this.taskServices.dateFromUTC(this.violation.dispositionDate, true);
      }
  ...
  get ecbNumber() {
    return this.formData.get('ecbNumber')
  }
  get deviceNumber() {
    return this.formData.get('deviceNumber')
  }
  get violationCategory() {
    return this.formData.get('violationCategory')
  }
  get dispositionDate() {
    return this.formData.get('dispositionDate')
  }
  ...
  */
  ...
  onSubmit() {
    ...
    if (this.idViolation) {
      ...
      this.jobViolationServices.updateDobViolation(this.idViolation, this.formData.value).subscribe(res => {
        console.log(res)
        this.loading = false;
        ...
        this.modalRef.hide();
        this.toastr.success('DOB Violation Updated!')
      }, err => {
        this.loading = false;
        this.toastr.error(err)
        console.log(err)
      ...
      this.jobViolationServices.createDobViolation(this.formData.value).subscribe(res => {
        console.log(res)
        this.loading = false;
        this.reload()
        this.modalRef.hide();
        this.toastr.success('DOB Violation Created!')
      }, err => {
        this.loading = false;
        this.toastr.error(err)
        console.log(err)
