...
export class JobApplicationDobSecondComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  callDOBNOWAPIs(jobNumber: any) {
    ...
    this.modalRef = this.modalService.show(this.viewdob, { class: 'modal-view-task', backdrop: 'static', 'keyboard': true })
  }
  ...
  private _openModalFormAddApplication(template: TemplateRef<any>) {
    this.modalRef = this.modalService.show(template, { class: 'modal-add-application', backdrop: 'static', 'keyboard': false })
  }
  ...
  /**
     ...
     */
  private openModalFormAddApplication(template: TemplateRef<any>, id?: number) {
    if (!id) {
      this.idJobApp = 0
    } else {
      this.idJobApp = id
