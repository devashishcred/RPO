...
import { Component, EventEmitter, Input, OnInit, Output, TemplateRef, ViewChild, ElementRef, OnDestroy } from '@angular/core';
...
export class AddTransMittalComponent implements OnInit, OnDestroy {
  ...
  */
  ...
  private modalRefDoc: BsModalRef
  ...
  private mailTo: number
  private mailType: number
  private mailVia: number
  private mailAttention: number = null;
  ...
  private jobTransmittalAttachments: any
  private disabled: boolean = false
  private dropdownEnabled: boolean = true
  private idCompany: number
  private jobDocumentList: any = []
  private documentsToDelete: any = []
  private attachmentsToDelete: any = []
  private currentRoute: string
  private messageFromTaskData: string = ''
  private treeViewConfig: any = {
    hasAllCheckBox: true,
    hasFilter: true,
  ...
  private draftToSend: boolean = false
  private contactCCItems: TreeviewItem[];
  private treeViewButtonClasses = [
    'btn-outline-primary',
  ...
  * @method getContacts
  */
  private getContacts(editTime?: boolean) {
  ...
  */
  private getCompanies() {
    this.transmittalService.getJobContactsCompanyListDD(this.idJob).subscribe(r => {
      this.companies = _.sortBy(r, function (i: any) { return i.itemName.toLowerCase(); });
  ...
  * @param {any} template type which contains template of create/edit module
  */
  private openDocumentModal(template: TemplateRef<any>) {
    this.modalRefDoc = this.modalService.show(template, { class: 'modal-doc-search', backdrop: 'static', 'keyboard': false })
  }
  ...
  */
  private getSelectedDocument(document: any) {
    if (document) {
 ...
 * @param {any} document document is an object of documents 
 */
  private getSelectedDocumentToDelete(documentsToDelete: any) {
  ...
  */
  private deleteJobDoc(id: number, idJobDocument?: number) {
    if (id) {
   ...
   * @method sendEmail
   */
  private sendEMail(isDraft?: boolean) {
