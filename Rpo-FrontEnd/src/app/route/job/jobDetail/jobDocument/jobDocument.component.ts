...
import { Component, EventEmitter, Input, OnInit, Output, TemplateRef, NgZone, ViewChild } from '@angular/core';
...
import { Pullpermit, PullPermitRequest } from './pullpermit';
...
@Component({
  selector: '[job-document]',
  templateUrl: './jobDocument.component.html',
  styleUrls: ['./jobDocument.component.scss']
})
export class JobDocumentComponent implements OnInit {
    ...
    vm.table = $('#dt-jobDocument').DataTable({
      ...
      ajax: (dataTablesParameters: any, callback: any) => {
        if (!vm.flager) {
          ...
          dataTablesParameters['start'] = dataTablesParameters['start'] + 100
          ...
          dataTablesParameters['orderedColumn'] = tempOrder;
      ...
      rowCallback: ((row: any, data: any, index: any) => {
        if (data.documentPath == '') {
          ...
          $(row).find('.clickable').removeClass('clickable');
        } else {
          $(row).find('.viewClass').show();
        }
        if(this.showJobDocumentDeleteBtn == 'hide') {
        ...
        this.specialColumn
          ...
          .onActionClick((row: any, actionId: any) => {
            ...
            if (actionId == "DeletePdf") {
              vm.delete(data.id, row, false, data)
            }
          ...
          $('#dt-jobDocument tbody').on('click', 'td.clickable', function (ev: any) {
  ...
  private openModalAddJobDocument(template: TemplateRef<any>, id?: number) {
