  ...
  private getAllEmployee() {
    if (!this.employee.length) {
      this.employeeServices.getAllEmployee().subscribe(employeeData => {
        let tmpEmployee = _.sortBy(employeeData.data, function (i: any) { return i.firstName.toLowerCase(); });
        this.employeeList = []
...
/**
  ...
  private getContacts() {
    if (!this.contacts.length) {
      this.contactServices.getCotactData().subscribe(r => {
        let tmpContact = _.sortBy(r.data, function (i: any) { return i.firstName.toLowerCase(); });
        this.contactList = []
  ...
  private getCompanies() {
    if (!this.companies.length) {
      this.companyServices.getCompanyData().subscribe(r => {
        this.companies = _.sortBy(r.data, function (i: any) { return i.name.toLowerCase(); });
      })
    }
  ...
  */
  private searchJobs(clearSearch?: boolean) {
