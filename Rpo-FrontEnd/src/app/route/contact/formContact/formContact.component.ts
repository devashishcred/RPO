...
export class FormContactComponent implements OnInit, OnDestroy {
  ...
  private contactLicense: ContactLicense
  private idxContactLicense: number = -1
  private alreadyMailAddress: any
  private showWarningMsg: boolean = false
  private showPersonalWarningMsg: boolean = false
  private disableThePrimaryCheckbox: boolean = false
  private disableThePersonalCheckbox: boolean = false

  private cities: City[] = []
  private states: State[] = []
  private prefixes: Prefix[] = []
  private suffixes: Suffix[] = []
  private contactTitles: ContactTitle[] = []
  private contactLicenses: ContactLicense[] = []
  private contactLicenseTypes: ContactLicenseType[] = []
    ...
    if (this.idCompany != null) {
      ...
      this.companyServices.getById(this.idCompany).subscribe(r => {
        ...
        for (let i = 1; i < this.companyAddresses.length; i++) {
          this.companyPrimaryAddresses.push({ id: this.companyAddresses[i].id, itemName: '[' + this.companyAddresses[i].addressType.name + '] ' + this.companyAddresses[i].address1 + ', ' + (this.companyAddresses[i].address2 != null ? this.companyAddresses[i].address2 + ', ' : "") + (this.companyAddresses[i].city != null ? this.companyAddresses[i].city + ', ' : "") + this.companyAddresses[i].state + ' ' + (this.companyAddresses[i].zipCode != null ? this.companyAddresses[i].zipCode : "") });
        }
      })
    }
  ...
  */
  private editContactLicense(cl: ContactLicense, idx: number) {
  ...
  */
  private deleteContactLicense(cl: ContactLicense) {
  ...
  */
  private editAddress(ad: Address, idx: number) {
    this.address = { ...ad }
    this.idxAddress = idx
  }
  ...
  */
  private deleteAddress(ad: Address) {
  ...
  */
  private deleteDocument(d: ContactDocument) {
    if (d.id) {
 ...
 */
  ...
  */
  private fetchAddressFromCompany() {
    if (this.rec.idCompany != null) {
      this.companyServices.getById(this.rec.idCompany).subscribe(r => {
        this.recCompany = r as Company
        this.companyAddresses = this.recCompany['addresses']
