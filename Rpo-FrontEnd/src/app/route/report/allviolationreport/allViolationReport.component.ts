...
export class AllViolationReportComponent implements OnInit {
	...
	private modalRefJobSelect: BsModalRef
	private filter: any = {}
   ...
   */
	...
	loadDataTable(callApi: boolean = false) {
		...
		this.filter.Type_ECB_DOB = this.violationType === 'AOTH Violation' ? "ECB" : 'DOB'
	...
	openCreateTransmittalModal(jobId?: number) {
		...
		if (this.isCustomerLoggedIn) {
			this.jobServices.getCustomerJobDetailById(jobId).subscribe(res => {
				this.loading = false;
				this.jobDetail = res
				this.modalRef = this.modalService.show(this.addtransmittal, { class: 'modal-add-transmittal', backdrop: 'static', 'keyboard': true })
			}, error => {
				this.loading = false;
			})
		} else {
