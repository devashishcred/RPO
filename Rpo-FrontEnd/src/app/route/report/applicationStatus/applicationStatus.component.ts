	...
	*/
	...
	loadDataTable() {
		...
		vm.table = $('#dt-application-status').DataTable({
			paging: true,
			dom: "<'row'<'col-sm-12'tr>>" + "<'row'<'col-sm-12 col-md-2'l><'col-sm-12 col-md-2'i><'col-sm-12 col-md-8'p>>",
			pageLength: 25,
			"bFilter": true,
			lengthChange: true,
			...
			"aaSorting": [],
			"order": [[0, "desc"]],
			ajax: this.reportServices.applicationStatusReport({
				onData: (data: any) => {
					assign(data, pickBy(this.filter, identity))
				}
			}),
			columnDefs: [
				{ type: 'date-uk', targets: [6, 7, 8, 9, 10] }, // NOTE specify your date column number,starting from 0        
			],
			columns: [
				{
					title: 'Project#',
				...
				{
					title: 'Work Type',
					data: 'jobWorkTypeCode',
					render: function (data: any, type: any, dataToSet: any) {
						let workType = "";
						if (dataToSet.idParent == 1 || dataToSet.idParent == 4) {
							workType = dataToSet.jobWorkTypeCode;
						} else if (dataToSet.idParent == 2) {
							workType = dataToSet.permitType;
						}
						return workType;
					},
					class: 'jobRedirect',
	...
	exportClick(type: string) {
		...
		this.loading = true;
		this.reportServices.downloadApplicationReport(this.filterForDownload, type).subscribe(r => {
			this.loading = false;
