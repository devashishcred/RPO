...
declare const $: any
/**
*  This component contains all function that are used in SeismicDesignCategoriesComponent
* @class SeismicDesignCategoriesComponent
*/
@Component({
  templateUrl: './seismicdesigncategories.component.html',
  styleUrls: ['./seismicdesigncategories.component.scss']
})
export class SeismicDesignCategoriesComponent implements OnInit, OnDestroy {

  /**
  *  seismicdesigncategoriesform add/edit form
  * @property seismicdesigncategoriesform
  */
  @ViewChild('seismicdesigncategoriesform',{static: true})
  private SeismicDesignCategoriesform: TemplateRef<any>
  ...
  private SeismicDesignCategoriesId: number
  private search: string
  private showDesignAddBtn: string = 'hide'
  private showDesignDeleteBtn: string = 'hide'
  ...
  constructor(
    ...
    this.showDesignAddBtn = this.userRight.checkAllowButton(this.constantValues.ADDMASTERDATA)
    this.showDesignDeleteBtn = this.userRight.checkAllowButton(this.constantValues.DELETEMASTERDATA)

    this.specialColumn = new $.fn.dataTable.SpecialColumn([{
      id: 'EDIT',
      title: 'Edit',
      customClass: this.showDesignAddBtn
    }, {
      id: 'DELETE',
      title: 'Delete',
      customClass: this.showDesignDeleteBtn
    }], false)
  ...
  */
  ngOnInit() {
    document.title = 'Seismic Design Categories'
    const vm = this
    this.filter = {} as any
    vm.table = $('#dt-JobContact-types').DataTable({
      ...
      "aaSorting": [],
      ajax: this.SeismicDesignCategoriesServices.getRecords({
        onData: (data: any) => {
      ...
      initComplete: () => {
        this.specialColumn
          ...
          .onActionClick((row: any, actionId: any) => {
            ...
            if (actionId == "EDIT") {
              vm.isNew = false
              vm.SeismicDesignCategoriesId = data.id
              vm.openModalForm(vm.SeismicDesignCategoriesform, data.id, false)
            }
            if (actionId == "DELETE") {
  ...
  * @method searchSeismicDesignCategories
  * @param {string} srch type any which contains string that can be filtered from datatable
  */
  private searchSeismicDesignCategories(srch: string) {
  ...
  */
  private openModalForm(template: TemplateRef<any>, id?: number, isNew?: boolean) {
    if (isNew) {
      this.isNew = true
      this.SeismicDesignCategoriesId = 0
    }
  ...
  */
  delete(id: number, row: any) {
    return new Promise((resolve, reject) => {
      this.SeismicDesignCategoriesServices.delete(id).subscribe(r => {
        row.delete()
        resolve(r)
      }, e => {
        reject()
