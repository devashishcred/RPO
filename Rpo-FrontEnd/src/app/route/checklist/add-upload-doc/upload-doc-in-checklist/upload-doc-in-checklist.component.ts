...
import { ModelTosent } from '../../../../types/document';
import { PW517Doc } from '../../../job/jobDetail/jobDocument/pullpermit';
...
@Component({
  selector: '[upload-doc-in-checklist]',
  templateUrl: './upload-doc-in-checklist.component.html',
  styleUrls: ['./upload-doc-in-checklist.component.css']
})
export class UploadDocInChecklistComponent implements OnInit {
  ...
  private maxRecords: boolean = false
  private errorMsg: any
  private showFieldValues: boolean = false
  private noSiaNumber: boolean = false
  ...
  private AllSelectedDays: any[] = [];
  private isDateExeed14Days: boolean = false;


  private pwDoc: PW517Doc;
  private pw517ApplicationList: any = [];
  private pw517VarianceList: any = [];
  private pw517ApplicantList: any = [];
  private pw517FillingTypeList: any = [];

  private FinalArray: any[] = [];
  private weekShowArray: any[] = [
    "sun",
    "mon",
    "tue",
    "wed",
  ...
  private onCalender: any;
  private showCalender: boolean = false;
  private selectedCalenderView: any[] = [];
  private parentValues: any = []
  private requireAHVRefForPW517: boolean = false
  private workPermitForPW517App: any = []
  ...
  ngOnInit() {
    ...
    if(this.IsPlGroup){
      this.ListOfFieldsTosent = {
        ...
        jobDocumentFields: [],
        IdJobPlumbingInspections : this.isChecklist
      } as ModelTosent;
    }else{
      this.ListOfFieldsTosent = {
        id: (this.DocumentId == null ? 0 : this.isClone == true ? 0 : this.DocumentId),
        ...
        documentName: (this.DocumentId == null ? '' : this.DocumentObj.documentName),
        jobDocumentFields: [],
        IdJobchecklistItemDetails: this.isChecklist
      } as ModelTosent;
    }
 ...
 */
  ...
  getDocumentId(){
    ...
    if(!this.IsPlGroup){
      this.jobDocumentServices.getChecklistDocIdForUpload(this.isChecklist).subscribe(r=>{
        if(r?.length>0)
        {  this.selectedDocument = r[0].idUploadFormDocument;
          this.getDocumentFields(this.selectedDocument);}
      
       })
    }else{
      this.jobDocumentServices.getChecklistDocIdForUploadForPl(this.isChecklist).subscribe(r=>{
        if(r?.length>0){
          this.selectedDocument = r[0].idUploadFormDocument;
          this.getDocumentFields(this.selectedDocument);
        }
...
*/
  saveDocument() {
    ...
    onlyFrontEndFields.forEach((element: any, key: number) => {
      ...
      //api/jobdocumentdrodown/JobApplicationNumberType/5100/57/0
    ...
    if (this.selectedDocument == "113" && this.AllSelectedDays && this.AllSelectedDays.length > 0) {
      ...
      if (tempAllDates.length == this.AllSelectedDays.length) {
        ...
        tempAllDates.forEach(function (v: any) {
          ...
          // check any date entered in weekday
          //if(this.is_weekend(v) && )
    ...
    if (!this.isDateExeed14Days) {
      this.jobDocumentServices.createJobDocument(this.ListOfFieldsTosent).subscribe(resOfDocument => {
        if (this.documents && this.documents.length > 0) {
          ...
          if (resOfDocument.documentPath && this.documents.length == 0 && (this.ListOfFields.filter(function (e: any) { return e.controlType === 9; }).length) == 0) {
  ...
  */
  ...
  // For PW-513 Document
  // resetDates(dates: any, fieldName: string) {
  //   if (fieldName == 'txtDate1' && !dates) {
  //     this.sundayArray = [];
  //   }
  //   else if (fieldName == 'txtDate2' && !dates) {
  //     this.mondayArray = [];
  //   }
  //   else if (fieldName == 'txtDate3' && !dates) {
  //     this.tuesdayArray = [];
  //   }
  //   else if (fieldName == 'txtDate4' && !dates) {
  //     this.wensdayArray = [];
  //   }
  //   else if (fieldName == 'txtDate5' && !dates) {
  //     this.thursdayArray = [];
  //   }
  //   else if (fieldName == 'txtDate6' && !dates) {
  //     this.fridayArray = [];
  //   }
  //   else if (fieldName == 'txtDate7' && !dates) {
  //     this.saturdayArray = [];
  //   }

  //   if (dates) {
  //     let datesArray = dates.split(',');
  //     if (fieldName == 'txtDate1') {
  //       this.sundayArray = datesArray;
  //     }
  //     else if (fieldName == 'txtDate2') {
  //       this.mondayArray = datesArray;
  //     }
  //     else if (fieldName == 'txtDate3') {
  //       this.tuesdayArray = datesArray;
  //     }
  //     else if (fieldName == 'txtDate4') {
  //       this.wensdayArray = datesArray;
  //     }
  //     else if (fieldName == 'txtDate5') {
  //       this.thursdayArray = datesArray;
  //     }
  //     else if (fieldName == 'txtDate6') {
  //       this.fridayArray = datesArray;
  //     }
  //     else if (fieldName == 'txtDate7') {
  //       this.saturdayArray = datesArray;
  //     }
  //   }
  //   this.AllSelectedDays = [];
  //   this.AllSelectedDays = this.mondayArray.concat(this.tuesdayArray,
  //     this.wensdayArray, this.thursdayArray, this.fridayArray, this.saturdayArray, this.sundayArray);
  //   this.setTimeValueInListObject('txtNo_Days', this.AllSelectedDays.length);
  // }
  ...
  savePW517Document() {
    ...
    // else if (!this.pwDoc.weekendDescription && () && !this.pwDoc.isSameAsWeekday) {
    ...
    if (!isValidation) {
      ...
      console.log("this.pwDoc",this.pwDoc);
  ...
  // sameWeekdayDescription (checked: boolean) {
  //     this.pwDoc.weekendDescription = checked?  this.pwDoc.weekdayDescription : '';
