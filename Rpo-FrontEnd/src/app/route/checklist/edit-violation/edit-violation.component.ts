...
export class EditViolationComponent implements OnInit {
  ...
  private disableGetInfo: string = 'hide'
  ...
  private disableSummons: boolean = false
    ...
    if (this.idViolation > 0) {
      ...
      this.jobViolationServices.getById(this.idViolation).subscribe(r => {
        ...
        if (this.violation.hearingTime) {
          // this.violation.hearingTime = this.taskServices.dateFromUTC(this.violation.hearingTime, true);
  ...
  getViolationInfo() {
    if (!this.violation.summonsNumber.match("^[a-zA-Z0-9]*$")) {
      ...
      this.jobViolationServices.getOATH(this.violation.summonsNumber).subscribe(r => {
        ...
        // this.violation.idViolationPaneltyCode = r.code
        // this.violation.paneltyCodeSection = r.codeSection
        // this.violation.description = this.toTitleCase(r.description)
        // r.paneltyAmount = r.paneltyAmount.replace('$', '');
        // this.violation.paneltyAmount = r.paneltyAmount
      }, e => {
        this.loading = false
      })
    }
