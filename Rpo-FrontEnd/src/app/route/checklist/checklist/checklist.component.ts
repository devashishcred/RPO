...
export class ChecklistComponent implements OnInit {
  ...
  private VoilationlistArray: any[] = [];
  private VoilationDoblistArray: any[] = [];
  panelOpenState = false;
  private modalRef: BsModalRef;
  private loading: boolean = false;
  private is_mail_sent: boolean = false;
  private idJob: number;
  private idViolation: number;
  public violationType: number;
  private checkListSwitcherData: any[] = [];
  private groupId: any;
  private selectedItemId: any;
  private IdJobChecklistItemDetail: any;
  ...
  dateEvent: any;
  private showChecklistAddBtn: string = 'hide';
  private showChecklistDeleteBtn: string = 'hide';
  private showAddViolation: string = 'hide'
  private showDeleteViolation: string = 'hide'
  ...
  mainListDrop(event: CdkDragDrop<string[]>) {
    ...
    console.log(data);
    data.map(r => { delete r.groups, delete r.isExpanded, delete r.isCOProject, delete r.isParentCheckList })
  ...
  private openModalReferenceNote(
    template: TemplateRef<any>,
    id?: number,
  ...
  private openModalAddDoc(template: TemplateRef<any>, id?: number) {
    this.IdJobChecklistItemDetail = id;
    this.modalRef = this.modalService.show(template, {
      class: "modal-job-document",
      backdrop: "static",
      ...
      this.jobCheckListServices.getChecklistAllCustomer(id, "GROUP", this.searchedTerm).subscribe(
        (r) => {
          ...
          r.forEach((r) => {
            ...
            r.groups.forEach((s) => {
              s.isExpanded = false;
              s.item.forEach((r) => (r.isExpanded = false));
   ...
   * @param {boolean} isNew it is optional which contains true if it is new record and false when it is old record
   */
  private openViolationModalForm(
    template: TemplateRef<any>,
    id?: number,
    isNew?: boolean
  ) {
   ...
   * @param {boolean} isNew it is optional which contains true if it is new record and false when it is old record
   */
  private openExportChecklistModalForm(
    template: TemplateRef<any>,
    exportFileType?: string
  ) {
